...
/**
 * This is a quick and dirty dev-standin (for local testing) that stores vehicle
 * position reports in a memory structure.
 * 
 * Thread safety is NOT OFFERED; remember this is intended to be used in local
 * development.
 * 
 * Use the database backed implementation in a production, clustered or cloud
 * environment.
 */
@Repository
@Profile({ "production-microservice", "standalone" })
public class DataPostgresImpl implements Data {
	...
	public void updatePosition(VehiclePosition data) {
		...
		try {
			...
			vehicleWithSpeed = new VehicleBuilder().withVehiclePostion(data).withSpeed(speed).withTimestamp(new Date())
					.build();
		...
		repository.save(vehicleWithSpeed);
	...
	public VehiclePosition getLatestPositionFor(String vehicleName) throws VehicleNotFoundException {
		...
		TreeSet<VehiclePosition> reportsForThisVehicle = new TreeSet<VehiclePosition>(repository.findByName(vehicleName));
		if (reportsForThisVehicle == null)
	...
	public TreeSet<VehiclePosition> getAllReportsForVehicleSince(String name, Date timestamp)
		...
		// Could use a Java 8 lambda to filter the collection but I'm playing safe in
		// targeting Java 7

		TreeSet<VehiclePosition> vehicleReports = new TreeSet<VehiclePosition>(repository.findByName(name));
	...
	public Set<VehiclePosition> getLatestPositionsOfAllVehiclesUpdatedSince(Date since) {
		...
		for (VehiclePosition vehicle: repository.findAll())
		{
